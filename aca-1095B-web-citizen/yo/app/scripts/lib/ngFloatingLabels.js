/**
 * Created by sdalavi on 9/29/2015.
 */
  ...
  .directive('floatingLabel', ['$animate', function($animate){
    return {
      ...
      link: function(scope, element, attrs, controller){
        ...
        var $float = "";
        if(attrs.placeholder.indexOf("(Optional)") >= 0){
          $float = jQuery('<label for="'+attrs.id+'" class="float">'+attrs.placeholder.substr(0,attrs.placeholder.indexOf("(Optional)")).trim()+'</span>');
        }else{
          $float = jQuery('<label for="'+attrs.id+'" class="float">'+attrs.placeholder+'</span>');
        }
        ...
        scope.$watch('inputCtrl.$modelValue', function (newValue) {
          if(newValue !== undefined && newValue !== null){
            if(attrs.placeholder.indexOf("(Optional)") >= 0){
              $float[0].innerText = attrs.placeholder.substr(0,attrs.placeholder.indexOf("(Optional)")).trim();
            }else{
              $float[0].innerText = attrs.placeholder;
            }
