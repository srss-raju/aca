...
public class ViewPrintInfo {
    ...
    private List<CoverageInfoDTO> coverageInfoDTOs = new ArrayList<>();
    ...
    public void setCorrectionIndicator(String correctionIndicator) {
        ...
        else
        {
            if (CommonDataConstants.FormStatus.FORM_STATUS_REGENERATE.equals(this.formStatus)) {
                this.correctionIndicator= "correctedstamp";
            } else {
                this.correctionIndicator= "none";
            }
        }
    }
    ...
    public void setSystemConfigValues(SourceSystemConfig configdata, boolean isuserFound)
    {
        logger.info("setSystemConfigValues start  "+configdata.getSupportViewPdf());
        this.bFormView = (configdata.getSupportViewPdf() == null) && ("YES".equalsIgnoreCase(configdata.getSupportViewPdf()));
        this.stateContactInfo = configdata.getNotFoundMessage();
        this.errorMessage = isuserFound ? null:configdata.getNotFoundMessage();
        this.stateContactInfo = configdata.getIncorrectAddressMessage();
        logger.info("setSystemConfigValues end  "+configdata.getSupportViewPdf());
    }



    public void setDataValues(FilerDemographicCP filerdemographic, String uidType, String corIndicator){

        logger.info("setDataValues start  ");
        this.correctionIndicator = corIndicator;
        this.uidType = uidType;
        ...
        this.policyOrigin =  filerdemographic.getPolicyOrigin();
        ...
        if(("SSN").equalsIgnoreCase(uidType))
        {
            if(filerdemographic.getRecepientSsn() != null)
                setUidNumber(getMaskedValue(filerdemographic.getRecepientSsn()));
        } else if(("TIN").equalsIgnoreCase(uidType) && filerdemographic.getRecepientTin() != null)
        {
        ...
        setSystemConfigValues(filerdemographic.getSourcesystemconfig(), filerdemographic.isUserFound());
    ...
    public void setCoveredPersonList(List<CoveredPerson> coveredPersonList)
    {
        for (CoveredPerson item : coveredPersonList) {
            CoverageInfoDTO coverageInfoDTO = new CoverageInfoDTO();
            coverageInfoDTO.setRecipientName(item.getName());
