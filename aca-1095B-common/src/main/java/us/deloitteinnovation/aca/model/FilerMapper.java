 ...
 */
@Component
public class FilerMapper implements RowMapper<Filer> {

    private static Logger logger = LoggerFactory.getLogger(FilerMapper.class);
    ...
    public Filer mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        if (correction != null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("CORRECTION_CODE for filer source cd {} and source unique id {} is NULL.  Setting to O and continuing.", filer.getSourceCd(), filer.getSourceUniqueId()) ;
            }
            filer.setCorrection("O");
        }
        return filer;
    }
